...
EVENT_ATTRS_MAPPING = {
    ...
    VkEventType.MESSAGE_NEW: [MSGID, 'flags', MSGID] + MESSAGE_EXTRA_FIELDS,
...
class VkLongPoll(object):
    ...
    def preload_message_events_data(self, events, extended=0):
        ...
        messages_data = self.vk.method('messages.getById', {
            'message_ids': ','.join(str(tmpId) for tmpId in message_ids),
            'extended': extended
        })
